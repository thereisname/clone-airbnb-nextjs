...
function FilterCategoriesModal({ isOpen, onClose }) {
  ...
  return (
    <div className={`fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-30 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div className={`bg-white py-5 px-5 my-12 rounded-lg w-full max-w-3xl max-h-full overflow-y-auto transform transition-transform duration-300 ${isOpen ? 'scale-100' : 'scale-95'}`}>
  ...
  <span className="flex-grow text-center">필터</span>
        ...
        <div className="mt-6 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-black text-white rounded-lg">적용</button>
        </div>
      </div>
    </div>
  );
